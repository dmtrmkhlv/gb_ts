  ...
  allItems.addEventListener("click",(e)=>{
    ...
    if(target.classList.contains('js-favorite')){
      if(target.classList.contains('active')){
        ...
        let index = newFavoriteData.findIndex((item) => item.id == target.dataset.id);
        if (index != -1) {
        ...
        localStorage.favoriteItems = JSON.stringify([...getFavoritesAmount('favoriteItems'), {id: target.dataset.id, name: target.dataset.name, image: target.dataset.image}]);
      }
    }
